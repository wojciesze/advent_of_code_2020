...
def read_input(i):
    for line in i.split("\n"):
        ...
        bag, contain = tuple(re.split('\s*bags contain\s*', line))
        data[bag] = {}
        if contain == 'no other bags.':
            continue

        for x in re.split('\s*,\s*', contain):
            m = re.match('(\d+) ([a-z ]+) bags*\.*$', x)
            if not m:
                raise KeyError('ala')
            number = m.group(1)
            color = m.group(2)
            data[bag][color] = number

## check if given bag can contain a color
def check_color(bag, color):
    main_bag = data[bag]
    if len(main_bag) < 1:
        return False
    for x in main_bag:
...
def part1():
    cnt = 0
    for x in data:
        if check_color(x, 'shiny gold'):
            cnt += 1
    print('[Part 1] magic number = ', cnt)

def part2(bag):
    if not data[bag]:
        return 0
    cnt = 0
    for _bag, _num in data[bag].items():
        cnt += int(_num) + int(_num) * part2(_bag)
    return cnt

if __name__ == '__main__':
    read_input(input)
    part1()
    print('[Part 2] magic number = ',part2('shiny gold'))
