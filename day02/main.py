...
def read_input():
    for x in input.split("\n"):
        if x == '' or x.startswith(" "):
            continue
        res = re.match(r'(\d+)-(\d+) ([a-z]): ([a-z]+)\s*$', x)
        if not res:
            print('Not match: ', x)
            continue
...
def part1():
    valid_passwords = 0
    for min, max, char, string  in data:
        if min <= count_chars(char, string) <= max:
            valid_passwords += 1
    print('[Part 1] Valid passwords: ', valid_passwords)

def part2():
    def valid(p1, p2, c, password):
        try:
            c1 = password[p1-1]
            c2 = password[p2-1]
        except IndexError:
            return False
        if (c == c1 and c != c2) or (c == c2 and c != c1):
            return True
        return False

    valid_passwords = 0
    for min, max, char, string in data:
        if valid(min, max, char, string):
            valid_passwords += 1

    print('[Part 2] Valid passwords: ', valid_passwords)
