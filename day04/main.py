...
def part1():
    ...
    for passport in passports:
        common = set(required_keys).intersection(set(passport))
        if len(common) == len(required_keys):
            valid_passports += 1
    print('[Part 1] Valid passports: ', valid_passports)


def validate_passport_part2(p):
    cnt = 0
    for field,regexp in required_keys.items():
        try:
            value = p[field]
        except KeyError:
            return False
        ...
        if field == 'hgt':
            unit = value[-2:]
            value = int(value[:-2])
            if (unit == 'cm' and (value > 193 or value < 150)) or (unit == 'in' and (value > 76 or value < 59)):
                return False
        cnt += 1
...
def part2():
    ...
    for passport in passports:
        if validate_passport_part2(passport):
            validate_passports += 1
    print('[Part 2] Valid passports: ',validate_passports)
