...
def read_input(i):
    ...
    for line in i.split("\n"):
        ...
        while cnt < len(line):
            el = line[cnt]
            if el in {'s', 'n'}:
                el_array.append(el + line[cnt+1])
                cnt += 1
            else:
                el_array.append(el)
            cnt += 1
        data.append(el_array)
        if len(el_array) > longest:
            longest = len(el_array)


if __name__ == '__main__':
    read_input(input)
    data2 = [[0 for _ in range(longest * 4 + 2)] for _ in range(len(data) * 4 + 2)]
    base_y = int(len(data2) / 2)
    base_x = int(longest * 2)
    for line in data:
        y = base_y
        x = base_x
        cnt = 0
        while cnt < len(line):
            ...
            if d == 'w':
                ...
                raise LookupError(f'Invalid direction: {d}')
            cnt += 1
            if cnt == len(line):
                if data2[y][x] == 1:
                    data2[y][x] = 0
